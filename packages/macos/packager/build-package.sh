...
copyBuildDirectory() {
    ...
    sed -i '' -e 's/__VERSION__/'${VERSION}'/g' ${TARGET_DIRECTORY}/darwin/scripts/com.mondoo.client.plist
    ...
    sed -i '' -e 's/__VERSION__/'${VERSION}'/g' ${TARGET_DIRECTORY}/darwinpkg/Library/${PRODUCT}/etc/com.mondoo.client.plist
...
function buildPackage() {
    ...
    pkgbuild --identifier com.mondoo.client \
    --version ${VERSION} \
...
function createInstaller() {
    ...
    buildPackage
    buildProduct ${PRODUCT}-macos-universal-${VERSION}.pkg
    #while true; do
...
function createUninstaller(){
    cp $SCRIPTPATH/darwin/Resources/uninstall.sh ${TARGET_DIRECTORY}/darwinpkg/Library/${PRODUCT}
